// taken from https://github.com/jerosoler/Drawflow
...
class Drawflow {
    ...
    export() {
        ...
        let start_node = null;
        let end_node = null;
        for (const key in dataExport.drawflow['Home'].data) {
            const item = document.getElementById('node-' + key);
            graph["node-" + key] = graph["node-" + key] || [];
            if (dataExport.drawflow['Home'].data[key]['inputs']['input_1'].connections.length === 0) {
                start_node = "node-" + key;
            }
            if (dataExport.drawflow['Home'].data[key]['outputs']['output_1'].connections.length === 0) {
                end_node = "node-" + key;
            }
            for (const output in dataExport.drawflow['Home'].data[key]['outputs']['output_1'].connections) {
                graph["node-" + key].push("node-" + dataExport.drawflow['Home'].data[key]['outputs']['output_1'].connections[output].node);
            }


            const content = item.querySelector('.modal-content');
            node_description["node-" + key] = node_description["node-" + key] || {};
            let selectedMethod = '';
            for (let i = 0; i < content.children.length; i++) {
                let childElement = content.children[i];
                if (childElement.id.endsWith("method-selector")) {
                    selectedMethod = childElement.value;
                    node_description["node-" + key]['stage'] = selectedMethod.split('-')[2];
                    node_description["node-" + key]['method'] = selectedMethod.split('-')[4];
                    break;
                }

            }
            let params = document.getElementById(selectedMethod);
            node_description["node-" + key]['params'] = {};
            for (let i = 0; i < params.children.length; i++) {
                let childElement = params.children[i];
                let param_name = childElement.id.split('-');
                node_description["node-" + key]['params'][param_name[param_name.length - 1]] = childElement.value;
            }
        }

        let result = {"graph": graph, "node_description": node_description, "start_node": start_node, "end_node": end_node};
        fetch("/run-pipeline", {
          method: 'POST', // or 'PUT'
          headers: {
